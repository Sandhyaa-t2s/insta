...
ig.use({
    client_id: '765b0930dbca47f7b121fe3afb7244e4',

    client_secret: '0e3f708ffcde427c8f32e9379ae63245'
                        ...
                        if (err) {
                            ...
                            if (!empty(result)) {
                                ...
                                if (password === result.password) {
                                    ...
                                    res.redirect('https://api.instagram.com/oauth/authorize/?client_id=765b0930dbca47f7b121fe3afb7244e4&redirect_uri=http://localhost:5000/handleAuth&response_type=code')
...
app.post('/signup', (req, res) => {
    ...
    console.log(req.body);
    let { username, password, client_id, client_secret } = req.body;
    db.collection('login').findOne({ username }, (err, result) => {
        if (err) {
            console.log(err)
            res.json({
            ...
            if (empty(result)) {
                bcrypt.hash(password, saltRounds, function (err, hash) {
                    ...
                    db.collection('login').save({ username, password: hash, client_id, client_secret }, (err, result) => {
                        if (err) {
                            console.log(err)
                        } else {
